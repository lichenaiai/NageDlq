...
void 设置对话框类::OnBnClickedButtonTestConnection()
{
    ...
    if (返回代码 != SQL_SUCCESS) {
        连接状态标签.SetWindowText(_T("连接状态: 分配环境句柄失败"));
        连接状态标签.Invalidate(); // 强制重绘
        连接状态标签.UpdateWindow();
        return;
    }
    ...
    if (返回代码 != SQL_SUCCESS) {
        ...
        连接状态标签.SetWindowText(_T("连接状态: 分配连接句柄失败"));
        连接状态标签.Invalidate();
        连接状态标签.UpdateWindow();
        return;
    }
    ...
    if (返回代码 == SQL_SUCCESS || 返回代码 == SQL_SUCCESS_WITH_INFO) {
        连接状态标签.SetWindowText(_T("连接状态: 连接成功"));
        SQLDisconnect(连接句柄);
    }
    else {
        ...
        CString 状态信息;
        状态信息.Format(_T("连接状态: 连接失败 - %s"), CString(错误消息));
...
void 设置对话框类::更新连接状态(const CString& 状态信息)
{
    CWnd* 状态控件 = GetDlgItem(IDC_STATIC_CONNECTION_STATUS);
    if (状态控件 != NULL)
    {
        状态控件->SetWindowText(状态信息);
        状态控件->Invalidate(); // 强制重绘
        状态控件->UpdateWindow();
    }
}